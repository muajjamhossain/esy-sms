    ...
    public function MarksStore(Request $request){
        ...
        if ($studentcount) {
            foreach ($request->student_id as $index => $studentId) {
                $query = StudentMarks::where('student_id', $studentId)
                    ->where('year_id', $request->year_id)
                    ->where('class_id', $request->class_id)
                    ->where('assign_subject_id', $request->assign_subject_id)
                    ->where('exam_type_id', $request->exam_type_id);
                if ($query->exists()) {
                    $existing[] = $request->id_no[$index] ?? $studentId;
                }
            }
        }
        if ($existing && ! $request->boolean('confirm_update')) {
            return redirect()->back()->withInput()->with('marks_duplicate_students', $existing);
        }
